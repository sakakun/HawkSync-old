...
namespace HawkSync_RC
{
    public partial class Main_Login : Form
    {
        ...
        private bool openedProfileList;
        private RC_Profilelist profileList;
        private readonly RCSetup RCSetup;
        public Timer Ticker;
        private readonly Thread updateInternal;
        ...
        {
            if (RCSetup.loopTrigger == false) return;
            if (RCSetup.client != null && RCSetup.client.Connected && RCSetup.clientStatus == OpenClass.Status.READY)
            {
                var Instances = clientClass.GetInstances(RCSetup.client, RCSetup.SessionID);
                if (Instances == string.Empty || RCSetup.TimedOut) return;
                try
                {
        ...
        public void UpdateInternal()
        {
            try
            {
                while (true)
                {
                    if (RCSetup.loopTrigger == false) break;
                    if (RCSetup.client != null && RCSetup.client.Connected &&
                        RCSetup.clientStatus == OpenClass.Status.READY)
                    {
                        var Instances = clientClass.GetInstances(RCSetup.client, RCSetup.SessionID);
                        if (Instances == string.Empty || RCSetup.TimedOut) continue;
                        try
                        {
                            var instancesReply = JsonConvert.DeserializeObject<Dictionary<dynamic, dynamic>>(Instances);
                            var RCInstancesReply = new RCInstances
        ...
        private void Ticker_Tick(object sender, EventArgs e)
        {
            ...
            if ((openedProfileList && RCSetup.client.Connected == false) || RCSetup.TimedOut)
            {
                openedProfileList = false;
        ...
        {
            // client options
            var clientOptions = new ClientOptions(_state, RCSetup);
        ...
        {
            // delete profile
            var profileInt = comboBox1.SelectedIndex;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(textBox2.Text) || string.IsNullOrWhiteSpace(textBox2.Text) ||
                !IPAddress.TryParse(textBox2.Text, out address))
            {
                MessageBox.Show("Please enter a valid Hostname or IP Address!", "Error");
                return;
            }

            if (numericUpDown1.Value < 0 || numericUpDown1.Value > 25565)
            {
            ...
            {
                var num = comboBox1.Items.Count;
                var newProfile = RCSetup.ProfileConfig.Configs.Add(textBox1.Text);
            ...
            else
            {
                var updateCurrentProfile =
                    RCSetup.ProfileConfig.Configs[comboBox1.Items[comboBox1.SelectedIndex].ToString()];
                ...
                RCSetup.ProfileConfig.Save();
                var updateProfile = ProgramConfig.RCProfiles[comboBox1.SelectedIndex - 1];
        ...
        private void click_connectServer(object sender, EventArgs e)
        {
            IPAddress address;
            if (string.IsNullOrEmpty(textBox1.Text) || string.IsNullOrWhiteSpace(textBox1.Text))
            {
            ...
            {
                var logoutStatus = clientClass.Logout(RCSetup.client, RCSetup.SessionID);
            ...
            else
            {
                ...
                var newpassword = Crypt.CreateMD5(ProgramConfig.RCProfiles[comboBox1.SelectedIndex - 1].Password);
                var username = ProgramConfig.RCProfiles[comboBox1.SelectedIndex - 1].Username;
                RCSetup.client = clientClass.Connect(ProgramConfig.RCProfiles[comboBox1.SelectedIndex - 1].Address,
                    ProgramConfig.RCProfiles[comboBox1.SelectedIndex - 1].Port);
                ...
                var openConn = clientClass.Open(RCSetup.client);
                ...
                var checkLoginStatus = clientClass.Login(RCSetup.client, RCSetup.SessionID, username, newpassword);
                if (checkLoginStatus == OpenClass.Status.LOGINSUCCESS)
                {
                    ...
                    var permissions =
                        JsonConvert.DeserializeObject<Dictionary<dynamic, dynamic>>(
                            clientClass.GetUserPermissions(RCSetup.client, RCSetup.SessionID));
                    ...
                    if (RCPerms.Status == OpenClass.Status.SUCCESS)
                    {
                        ...
                        var CountryCodes = clientClass.GetCountryCodes(RCSetup.client, RCSetup.SessionID);
                        ...
                        var autoRes = clientClass.GetAutoRes(RCSetup.client, RCSetup.SessionID);
        ...
        {
            var systemInfo = new SystemInfoClass();
            var MOS = new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_OperatingSystem");
            foreach (ManagementObject MOS_info in MOS.Get())
            {
                ...
                systemInfo.VirtualMemory =
                    Convert.ToString(Convert.ToInt32(MOS_info["TotalVirtualMemorySize"]) / 1024 / 1024) + "GB";
            }
        ...
        private void Main_Login_Load(object sender, EventArgs e)
        {
            ...
            var CurrentList = RCSetup.ProfileConfig.Configs;
        ...
        {
            var profileList = new RC_Profilelist(_state, RCSetup);
            Hide();
            profileList.ShowDialog();
            Show();
        }
