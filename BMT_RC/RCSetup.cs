    ...
    public class RCSetup
    {
        ...
        public byte[] SendCMD(Dictionary<dynamic, dynamic> cmd)
        {
            try
            {
                ...
                clientCMD.Connect();
                var response = clientCMD.SendAndWait(3000,
                    Compression.Compress(Encoding.Default.GetBytes(JsonConvert.SerializeObject(cmd))));
                var compressResponse = Compression.Decompress(response.Data);
        ...
        public byte[] SendCMD(Dictionary<string, dynamic> cmd)
        {
            try
            {
                ...
                clientCMD.Events.StreamReceived += Events_StreamReceived;
                clientCMD.Connect();
                var response = clientCMD.SendAndWait(30000,
                    Compression.Compress(Encoding.Default.GetBytes(JsonConvert.SerializeObject(cmd))));
                var compressResponse = Compression.Decompress(response.Data);
                clientCMD.Disconnect();
                clientCMD.Dispose();
