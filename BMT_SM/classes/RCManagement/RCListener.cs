            ...
            try
            {
                ...
                switch (json.action)
                {
                    ...
                    case "BMTRC.SkipMap":
                        if (_state.rcClients[json.SessionID].authenticated == true)
                        {
                            reply.Add("Status", RCFunctions.SkipMap(json.serverID, json.SessionID));
                        }
                        else
                        {
                            reply.Add("Status", RCListenerClass.StatusCodes.NOTAUTHENTICATED);
                        ...
                        var jsonSkipMap = JsonConvert.SerializeObject(reply);
                        return Encoding.ASCII.GetBytes(jsonSkipMap);
                    case "BMTRC.ScoreMap":
                        if (_state.rcClients[json.SessionID].authenticated == true)
                        {
